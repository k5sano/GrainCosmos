...
struct GrainVoice
{
    ...
    bool reversed = false;          // Reverse playback flag
    float attackPhase = 0.0f;       // Envelope attack progress (0-1)
    float decayPhase = 1.0f;        // Envelope decay progress (0-1)
    bool active = false;            // Whether this voice is currently playing
};

class GrainCosmosAudioProcessor : public juce::AudioProcessor
{
public:
    GrainCosmosAudioProcessor();
    ~GrainCosmosAudioProcessor() override;
    ...
    const juce::String getName() const override { return "GrainCosmos"; }
...
private:
    ...
    // DSP Components (declared BEFORE APVTS for proper initialization order)
    juce::dsp::ProcessSpec spec;

    // Grain buffer (circular delay line with Lagrange interpolation)
    ...
    // Freeze state controller (Phase 4.3)
    std::atomic<bool> freezeEnabled{false};
    float inputGainRamp = 1.0f;           // For smooth unfreeze transition
    int samplesSinceUnfreeze = 0;
    static constexpr int unfreezeRampSamples = 2200;  // 50ms at 44.1kHz
    ...
    float getADSREnvelope(float windowPos, float envelopeShape);  // Phase 4.2
    float applyDistortion(float sample, float distortionAmount);   // Phase 4.3
    float getQuantizedDelayTime(float delayTimeSec, bool tempoSync); // Phase 4.4

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(GrainCosmosAudioProcessor)
};
